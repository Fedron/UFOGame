...
public class WorldGenerator : MonoBehaviour {
	...
	private void SpawnBuildings() {
		for	(int i = 0; i < maxBuildings; i++) {
			...
			// for (int j = 0; j < 3; j++) {
			// 	spawnPos = new Vector3Int(Random.Range(0, width), Random.Range(0, height), 0);
			// 	Collider2D col = Physics2D.OverlapBox(new Vector2(spawnPos.x, spawnPos.y), collider.bounds.size, 0f);
			// 	if (!col) {
			// 		foundValidPos = true;
			// 		break;
			// 	}
